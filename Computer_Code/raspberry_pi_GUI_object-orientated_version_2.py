...
from constants_version_2 import *
...
class AlarmClock:
    ...
    def init_start_frame(self, master):
        ...
        image = ImageTk.PhotoImage(Image.open("eyesBaku.png").resize((160, 228)))  # Variables without the "self." can only be referenced in the method (local variable)
    ...
    def init_main_frame(self, master):
        ...
        set_hour_drop.grid(row=1, column=1, padx=(0,0), pady=(30,5))
        ...
        set_min_drop.grid(row=1, column=2, padx=(0,10), pady=(30,5))

        # Select day
        self.set_day_click = StringVar(self.main_frame, INITIAL_DAY)  # This variable stores the value selected in the combobox (same for a time range)
        set_day_drop = ttk.Combobox(self.main_frame, width=10, textvariable=self.set_day_click, values=DAYS)
        set_day_drop.grid(row=1, column=3, padx=(0,0), pady=(30,5))
        ...
        range_hour_drop.grid(row=5, column=1, padx=(0,0), pady=5)
        ...
        range_min_drop.grid(row=5, column=2, padx=(0,10), pady=5)

        # Select day
        self.range_day_click = StringVar(self.main_frame, INITIAL_DAY)  # This variable stores the value selected in the combobox (same for a time range)
        range_day_drop = ttk.Combobox(self.main_frame, width=10, textvariable=self.range_day_click, values=DAYS)
        range_day_drop.grid(row=5, column=3, padx=(0,0), pady=5)

        # Button to set the alarm time (passes input to "alarm_time" function)
        set_time_button = Button(self.main_frame,
                                       text="Set Alarm Time",
                                       command=lambda: self.alarm_time(
                                           self.set_time_label,
                                           self.set_day_click.get(),
                                           self.set_hour_click.get(),
                                           self.set_min_click.get(),
                                           "set_alarm_time"),
                                       relief="raised",
                                       background='#fffdd0')
        set_time_button.grid(row=2, column=1, padx=(15,0), pady=(10,10))

        # Set alarm time label
        self.set_time_label = Label(self.main_frame, text=" ", background='#a33651')
        self.set_time_label.grid(row=3, column=1, padx=(15,0), pady=(0,15))

        # Button to define a range alarm time (passes input to "alarm_time" function)
        range_time_button = Button(self.main_frame,
                                         text="Range Alarm Time",
                                         command=lambda: self.alarm_time(
                                             self.range_time_label,
                                             self.range_day_click.get(),
                                             self.range_hour_click.get(),
                                             self.range_min_click.get(),
                                             "range_alarm_time"),
                                         relief="raised",
                                         background='#fffdd0')
        range_time_button.grid(row=6, column=1, padx=(20,0), pady=(10,10))

        # Range alarm time label
        self.range_time_label = Label(self.main_frame, text=" ", background='#a33651')
        self.range_time_label.grid(row=7, column=1, padx=(15,0), pady=(0,50))
        ...
        raise_start_frame_button = Button(self.main_frame, text="Clock",
                                          command=lambda: self.raise_frame(self.start_frame), background='#fffdd0')
    ...
    def alarm_time(self, time_label, day, hour_click, min_click, mode):
        ...
        if day == "today":
            ...
            day_date = (self.t.day+1)
        time_label.config(text=f"Alarm set for {hour_click}:{min_click}")
        seconds_wait = self.count_seconds(int(day_date), int(hour_click), int(min_click))
